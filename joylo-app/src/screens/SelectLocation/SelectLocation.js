    ...
    <>
      <View style={styles().flex}>
        <View style={styles().mapView}>
          <MapView ref={mapRef} initialRegion={coordinates} region={coordinates} style={{ flex: 1 }} provider={PROVIDER_DEFAULT} showsTraffic={false} maxZoomLevel={15} customMapStyle={customMapStyle} onRegionChangeComplete={onRegionChangeComplete} />
          <View style={styles().mainContainer}>
            <CustomMarker width={40} height={40} transform={[{ translateY: -20 }]} translateY={-20} />
          </View>
        </View>
        <View style={styles(currentTheme).container}>
          <TextDefault textColor={currentTheme.newFontcolor} H3 bolder isRTL style={styles(currentTheme).heading}>
            {t('select_location')}
          </TextDefault>

          <TouchableOpacity activeOpacity={0.7} style={styles(currentTheme).button} onPress={setCurrentLocation}>
            ...
            {loading && <Spinner size={'small'} backColor={currentTheme.color3} spinnerColor={currentTheme.main} />}
          </TouchableOpacity>

          <View style={styles(currentTheme).line} />

          <TouchableOpacity activeOpacity={0.7} style={styles(currentTheme).button} onPress={() => setModalVisible(true)}>
            <View style={styles(currentTheme).icon}>
      ...
      <ModalDropdown theme={currentTheme} visible={modalVisible} onItemPress={onItemPress} onClose={() => setModalVisible(false)} />
    </>
  )
}
