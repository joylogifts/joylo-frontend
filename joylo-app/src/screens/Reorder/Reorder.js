...
function Reorder(props) {
  ...
  const onAddToCart = async () => {
    ...
    selectedItems.forEach(async (index) => {
      ...
      await addCartItem(item.food, item.variation._id, item.quantity, addons, index === 0)
    })
  ...
  return (
    <>
      <ScrollView showsVerticalScrollIndicator={false} alwaysBounceVertical={false} contentContainerStyle={styles(currentTheme).scrollViewStyle}>
        <View style={styles(currentTheme).mainContainer}>
          ...
          {order.items.map((item, index) => {
            ...
            return <ReorderItem key={food?._id} quantity={food?.quantity} dealName={food?.title} optionsTitle={food?.optionsTitle} itemImage={food?.image} itemAddons={food?.addons} dealPrice={(parseFloat(food?.price) * food?.quantity).toFixed(2)} checked={selectedItems.includes(index)} onPress={() => onSelect(index)} />
          })}
        </View>
