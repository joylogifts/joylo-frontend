...
import { BorderlessButton, FlatList, RectButton, TouchableOpacity } from 'react-native-gesture-handler'
...
function ImageTextCenterHeader(props, ref) {
  ...
  const aboutObject = {
    ...
    isOpen: () => {
      ...
      const todaysTimings = props.restaurant.openingTimes.find((o) => o.day === DAYS[day])
    ...
    >
      ...
      >
        ...
        <Animated.View style={styles().overlayContainer}>
          ...
          <Animated.View style={[styles().fixedView, { opacity: props.opacity }]}>
            <View style={styles().fixedText}>
              <TextDefault H4 bolder Center textColor={currentTheme.fontWhite} numberOfLines={1} ellipsizeMode='tail'>
                {aboutObject.restaurantName.length > 12 ? `${aboutObject.restaurantName.slice(0, 15)}...` : aboutObject.restaurantName}
              </TextDefault>

              {!props.loading && (
                <View style={styles(currentTheme).deliveryBox}>
                  <TextDefault style={[alignment.PRxSmall, alignment.PLxSmall]} textColor='white' bold>
                    {t('delivery')} {aboutObject.deliveryTime} {t('Min')}
                  </TextDefault>
                </View>
              )}
              {!props.loading && (
                ...
                >
                  <MaterialIcons name='star' size={scale(15)} color={currentTheme.white} />
                  <TextDefault style={[alignment.PRxSmall, alignment.PLxSmall]} textColor='white' bold>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={props.selectedLabel === index ? styles(currentTheme).activeHeader : null}>
              <RectButton activeOpacity={0.05} rippleColor={currentTheme.rippleColor} onPress={() => props.changeIndex(index)} style={styles(currentTheme).headerContainer}>
                <View style={styles().navbarTextContainer}>
                  <TextDefault style={props.selectedLabel === index ? textStyles.Bolder : textStyles.Small} textColor={props.selectedLabel === index ? currentTheme.tagColor : currentTheme.fontMainColor} center uppercase small>
                    {item.title}
                  </TextDefault>
                </View>
              </RectButton>
