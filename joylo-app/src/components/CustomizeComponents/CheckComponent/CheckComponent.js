...
function CheckComponent(props) {
  ...
  const [options, setOptions] = useState(props?.options?.map((option) => ({ ...option, checked: false })))
  ...
  return (
    <View>
      {options?.map((option) => (
        <TouchableOpacity activeOpacity={0.7} onPress={onPress.bind(this, option)} key={option._id} style={styles(currentTheme).mainContainer}>
          <View style={styles(currentTheme).leftContainer}>
            <CheckboxBtn onPress={onPress.bind(this, option)} checked={option.checked} />
            <TextDefault
              numberOfLines={1}
          ...
          <View style={styles(currentTheme).rightContainer}>
            <TextDefault textColor={currentTheme.gray900} H6 bolder>{`${configuration.currencySymbol}${option.price}`}</TextDefault>
          </View>
        </TouchableOpacity>
      ))}
    </View>
