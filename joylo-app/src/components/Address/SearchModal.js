...
import { Modal, View, TouchableOpacity, Keyboard, Dimensions, StyleSheet } from 'react-native'
import * as Location from 'expo-location'
import { GooglePlacesAutocomplete } from 'react-native-google-places-autocomplete'
import Animated, { Easing, Extrapolation, interpolate, useSharedValue, withTiming, useAnimatedStyle } from 'react-native-reanimated'
...
const getAutocompleteStyles = (currentTheme) =>
  StyleSheet.create({
    listView: {
      marginLeft: -scale(50)
    },
...
const useKeyboardAnimation = () => {
  ...
  const animateModal = (showKeyboard = false) => {
    animation.value = withTiming(showKeyboard ? 1 : 0, { duration: 300, easing: Easing.inOut(Easing.ease) })
  }

  const animatedContainerStyle = useAnimatedStyle(() => {
    'worklet'
    const interpolatedMarginTop = interpolate(animation.value, [0, 1], [marginTopHidden, marginTopShown], Extrapolation.CLAMP)
    const interpolatedBorderRadius = interpolate(animation.value, [0, 1], [initialBorderRadius, 0], Extrapolation.CLAMP)
...
export default function SearchModal({ visible = false, onClose = () => {}, onSubmit = () => {} }) {
  ...
  const renderPlaceRow = (data) => (
    <View key={data.place_id || data.id} style={{ flexDirection: 'row', alignItems: 'center', gap: scale(8) }}>
  ...
  return (
    <Modal visible={visible} transparent animationType={'slide'} onRequestClose={handleCloseModal}>
      <Animated.View style={[componentStyles.modalContainer, animatedContainerStyle]}>
        <View style={[componentStyles.flex, alignment.MTsmall, { padding: scale(16) }]}>
          <TouchableOpacity style={componentStyles.modalTextBtn} onPress={handleCloseModal}>
            <AntDesign name='arrowleft' size={scale(24)} color={currentTheme.newIconColor} />
          </TouchableOpacity>
