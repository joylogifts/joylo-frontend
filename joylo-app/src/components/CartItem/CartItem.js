  ...
  return (
    <View style={styles(currentTheme).itemContainer}>
      ...
      >
        ...
        <View>
          <TextDefault numberOfLines={1} textColor={currentTheme.fontFourthColor} bolder H5 isRTL>
            {typeof props?.dealName === "object" ? props?.dealName[selectedLanguage]?.length > 20 ? props?.dealName[selectedLanguage].substring(0, 17) + '...' : props?.dealName[selectedLanguage] : props?.dealName?.length > 20 ? props?.dealName.substring(0, 17) + '...' : props?.dealName}
          </TextDefault>
          ...
          {props?.itemAddons?.length > 0 && (
            <View style={styles().additionalItem}>
              <View>
                ...
                >
                  <TextDefault style={{ marginRight: scale(5) }} textColor={currentTheme.secondaryText} Normal isRTL>
                    {props?.optionsTitle?.slice(0, 3)?.length} {t('additional_items')}
                  </TextDefault>
          ...
          >
            ...
            <TouchableOpacity onPress={navigateBack}>
              <TextDefault textColor={currentTheme.fontFourthColor} bolder Normal isRTL>
                {t('edit')}
              </TextDefault>
            </TouchableOpacity>
          </View>
      ...
      <View style={styles(currentTheme).actionContainer}>
        <TouchableOpacity activeOpacity={0.7} style={[styles(currentTheme).actionContainerBtns, styles(currentTheme).minusBtn]} onPress={props?.removeQuantity}>
          {props?.quantity < 2 ? <EvilIcons name='trash' size={scale(25)} color={currentTheme.color4} /> : <AntDesign name='minus' size={scale(18)} color={currentTheme.color4} />}
        </TouchableOpacity>

        <Animated.View style={[styles(currentTheme).actionContainerView, animatedStyle]}>
          <TextDefault H5 bold textColor={currentTheme.black} isRTL>
            {props?.quantity}
          </TextDefault>
        </Animated.View>

        <TouchableOpacity activeOpacity={0.7} style={[styles(currentTheme).actionContainerBtns, styles(currentTheme).plusBtn]} onPress={props?.addQuantity}>
