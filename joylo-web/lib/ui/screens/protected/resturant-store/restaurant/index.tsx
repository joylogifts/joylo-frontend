...
export default function RestaurantDetailsScreen() {
  ...
  const { data: populareItemsList } = useQuery(
  ...
  const popularDealsIds = populareItemsList?.popularItems?.map(
    (item: any) => item.id
  );
  ...
  useEffect(() => {
    if (deals.length > 0 && !selectedCategory.title) {
      setSelectedCategory((prev) => ({
        ...prev,
        _id: deals[0]._id,
        title: toSlug(deals[0]?.title),
      })); // first category selected by default
    }
  }, [deals, selectedCategory.title]);
  ...
  // Handlers
  const handleScroll = (_id: string, title: string) => {
    ...
    selectedCategoryRef.current = title;
