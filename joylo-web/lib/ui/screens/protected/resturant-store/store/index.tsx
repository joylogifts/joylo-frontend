...
export default function StoreDetailsScreen() {
    ...
    const [subCategoriesForCategories, setSubCategoriesForCategories] =
        useState<ICategoryDetailsResponse[]>([]);
            ...
            allDeals
                .map((category: ICategory, index: number) => {
                    const subCats = subCategories.filter(
                        (sc: ISubCategory) =>
                            sc.parentCategoryId === category._id
                    );
                    ...
                    category.foods.forEach((food) => {
                        ...
                        if (!groupedFoods[subCatId])
                            groupedFoods[subCatId] = [];
            ...
            for (const popular of popularItems) {
                for (const category of allDealCategories) {
                    for (const subCat of category.subCategories) {
                        const match = subCat.foods.find(
                            (food) => food._id === popular.id
                        );
                        if (
                            match &&
                            !popularFoods.find((f) => f._id === match._id)
                        ) {
                  ...
                  [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
                        >
                            <ul className="flex space-x-4 items-center w-max flex-nowrap">
                                {menuItems?.map(
                                    (
                                        category: ICategoryDetailsResponse,
                                        index: number
                                    ) => {
                ...
                subcategoriesLoading ? (
                    ...
                    <div className="flex flex-col md:flex-row w-full">
                        ...
                        <div className="w-full md:w-4/5 p-3 h-full overflow-y-auto">
                            {deals.map(
                                (category: ICategoryV2, catIndex: number) => (
                                    <div
                                        key={catIndex}
                                        className="mb-4"
