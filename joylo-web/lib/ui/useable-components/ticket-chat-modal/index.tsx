...
export default function TicketChatModal({
    ...
    const pollingIntervalRef = useRef<ReturnType<typeof setTimeout> | null>(
        null
    );
        ...
        useQuery(GET_TICKET_MESSAGES, {
            ...
            onError: (error) => {
                showToast({
                    ...
                    message:
                        error.message ||
                        getTranslation("failed_to_load_messages"),
                });
            },
        });
    ...
    const [sendMessage] = useMutation(CREATE_TICKET_MESSAGE, {
        ...
        onError: (error) => {
            ...
            showToast({
                ...
                title: getTranslation("error_label"),
                message:
                    error.message || getTranslation("failed_to_send_message"),
            });
        },
    });
    ...
    const formatTimestamp = (timestamp: string) => {
        try {
            ...
            return getTranslation("unknown_time");
        }
    };
    ...
    const getTicketTitle = () => {
        if (!ticket) return getTranslation("support_chat_title");
        if (ticket.category === "order related" && ticket.orderId) {
            return `${getTranslation("order_issue_title")} - ${ticket.orderId}`;
        }
        return ticket.title;
    };
            ...
            <div className="flex flex-col md:h-[600px] h-[500px]">
                ...
                {ticketData?.getSingleSupportTicket?.description && (
                    <div className="border-b border-gray-200 p-3 bg-gray-50">
                        <div className="text-xs font-medium text-gray-500 mb-1">
                            {getTranslation("ticket_description_label")}:
                        </div>
                        ...
                        <div className="text-xs text-right mt-1 text-gray-500">
                            {formatTimestamp(
                                ticket?.createdAt || Date.now().toString()
                            )}
                        </div>
                    </div>
                )}
                    ...
                    {loading || ticketLoading ? (
                        ...
                        <div className="flex justify-center items-center h-full">
                            <p className="text-gray-500">
                                {getTranslation("no_messages_yet")}
                            </p>
                        </div>
                    )}
                </div>
                ...
                {isClosed ? (
                    <div className="p-4 border-t border-gray-200 bg-gray-50 text-center">
                        <p className="text-gray-500">
                            {getTranslation("ticket_closed_cannot_send")}
                        </p>
                    </div>
                ) : (
                        ...
                        <div className="flex gap-2">
                            <textarea
                                ...
                                placeholder={getTranslation(
                                    "type_your_message_here_placeholder"
                                )}
                                className="flex-1 p-3 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-[#FFA500] resize-none"
                                rows={2}
                                disabled={isSending}
                            />
                            <button
