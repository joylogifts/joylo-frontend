...
export default function PastOrders({
    ...
    function onCompleted() {
        showToast({
            type: "success",
            title: getTranslation("rating_label"),
            message: getTranslation("rating_submitted_successfully_message"),
            duration: 3000,
        });
    ...
    function onError() {
        showToast({
            type: "error",
            title: getTranslation("rating_label"),
            message: getTranslation("failed_to_submit_rating_message"),
            duration: 3000,
        });
        setSelectedOrder(null);
    }
    ...
    const handleReOrderClicked = useDebounceFunction(
        (
            ...
            router.push(
                `/${shopType == "restaurant" ? "restaurant" : "store"}/${slug}/${restaurantId}`
            );
        },
    ...
    if (pastOrders?.length === 0) {
        return (
            <EmptyState
                ...
                title={getTranslation("no_past_orders_title")}
                message={getTranslation("no_past_orders_message")}
                actionLabel={getTranslation("browse_store_button")}
                actionLink="/store"
            />
        );
    }
    ...
    return (
        <>
            <div className="space-y-4 py-4">
                <TextComponent
                    text={getTranslation("past_orders_label")}
