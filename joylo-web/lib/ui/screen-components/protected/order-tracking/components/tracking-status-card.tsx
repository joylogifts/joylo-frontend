            ...
            case "PENDING":
                ...
                if (orderTrackingDetails.createdAt) {
                    const createdTime = new Date(
                        orderTrackingDetails.createdAt
                    );
            ...
            case "CANCELLED":
                ...
                if (orderTrackingDetails.cancelledAt) {
                    const cancelledTime = new Date(
                        orderTrackingDetails.cancelledAt
                    );
            ...
            case "ASSIGNED":
                ...
                if (orderTrackingDetails.assignedAt) {
                    const assignedTime = new Date(
                        orderTrackingDetails.assignedAt
                    );
            ...
            case "ACCEPTED":
                if (orderTrackingDetails.preparationTime) {
                    const prepTime = new Date(
                        orderTrackingDetails.preparationTime
                    );
                    if (prepTime > now) {
                ...
                if (orderTrackingDetails.acceptedAt) {
                    const acceptedTime = new Date(
                        orderTrackingDetails.acceptedAt
                    );
            ...
            <div className="flex justify-between items-center mb-3">
                ...
                {orderTrackingDetails.orderStatus === "CANCELLED" && (
                    <span className="px-2 py-0.5 bg-red-100 text-red-800 rounded-full text-xs">
                        {getTranslation("order_status_cancelled_label")}
                    </span>
                )}
            </div>
            ...
            {orderTrackingDetails.orderStatus !== "DELIVERED" &&
                ...
                orderTrackingDetails.orderStatus !== "CANCELLED" && (
                    <div className="mt-2 flex items-center text-xs text-gray-500">
                        ...
                        {getTranslation("live_updates_enabled_label")}
                    </div>
                )}
        </div>
    );
